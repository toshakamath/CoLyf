...
export default class Daily extends Component {
  ...
  getData = () => {
    Promise.all([axios({
      method: 'get',
      url: '/assignment/daily?homeId=' + localStorage.getItem("homeId"),
    }),
    axios({
      method: 'get',
      url: '/task?homeId=' + localStorage.getItem("homeId") + "&rotationType=DAILY",
    })]
    )
      .then(response => {
    ...
    return (
      <div>
        <Row gutter={24}>
          ...
          <Col span={10}>
            <Title level={4}>All Daily Assignments</Title>
              {
                loading ?
                  <Skeleton active /> :
                  tasks.length > 0 ?
          ...
          <Col offset={4} span={10}>
            <Title level={4}>All Daily tasks</Title>
              {
                loading ?
                  <Skeleton active /> :
                  allTasks.length > 0 ?
                    ...
                    :
                    <Empty description="No daily tasks" />
              }
          </Col>
        </Row>
      </div>
