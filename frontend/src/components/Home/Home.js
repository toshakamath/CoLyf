...
export default class Home extends Component {
  ...
  getData = () => {
    Promise.all([axios({
      method: 'get',
      url: '/assignment/all?homeId=' + localStorage.getItem("homeId"),
    }),
    axios({
      method: 'get',
      url: '/task?homeId=' + localStorage.getItem("homeId") + "&rotationType=ALL",
    })]
    )
      .then(response => {
  ...
  handleAssignTask = (taskId) => {
    ...
    axios({
      ...
      .then(response => {
        console.log(response)
        this.getData();
        message.success("Task was assigned successfully")
      })
      .catch(err => {
        console.log(err)
      })
    ...
    return (
      <div>
        <Row gutter={24}>
          ...
          <Col span={10}>
            <Title level={4}>All Assignments</Title>
            {
              loading ?
                <Skeleton active /> :
                tasks.length > 0 ?
            ...
            <Title level={4}>All tasks</Title>
            {
              loading ?
                ...
                allTasks.length > 0 ?
                  ...
                  :
                  <Empty description="No all tasks" />
            }
          </Col>
        </Row>
      </div>
