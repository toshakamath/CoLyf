...
export default class Monthly extends Component {
  ...
  getData = () => {
    Promise.all([axios({
      method: 'get',
      url: '/assignment/monthly?homeId=' + localStorage.getItem("homeId"),
    }),
    axios({
      method: 'get',
      url: '/task?homeId=' + localStorage.getItem("homeId") + "&rotationType=MONTHLY",
    })]
    )
      .then(response => {
    ...
    return (
      <div>
        <Row gutter={24}>
          ...
          <Col span={10}>
            <Title level={4}>All Monthly Assignments</Title>
              {
                loading ?
                  <Skeleton active /> :
                  tasks.length > 0 ?
          ...
          <Col offset={4} span={10}>
            <Title level={4}>All Monthly tasks</Title>
              {
                loading ?
                  <Skeleton active /> :
                  allTasks.length > 0 ?
                    ...
                    :
                    <Empty description="No monthly tasks" />
              }
          </Col>
        </Row>
      </div>
